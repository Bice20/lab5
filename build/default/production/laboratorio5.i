...
; -------------- MACRO ---------------
  ...
  RESETTIMER0 MACRO
    ...
    BCF ((INTCON) and 07Fh), 2 ;llamar de bandera
    ENDM
...
isr:
    ...
    BTFSC ((INTCON) and 07Fh), 0 ; Fue interrupción? No=0 Si=1
...
CONFIG_IO:
    ...
    BSF WPUB, UP ;habilita el registro pullup en ((PORTB) and 07Fh), 0
    BSF WPUB, DOWN ;habilita el registro pullup en ((PORTB) and 07Fh), 1
...
CONFIG_INT:
    BANKSEL INTCON
    BSF ((INTCON) and 07Fh), 7 ;se habilita las interrupciones globales
    BSF ((INTCON) and 07Fh), 3 ;habilita interrupciones de cambio de estado del PORTB
    BCF ((INTCON) and 07Fh), 0 ;se limpia la bandera
    BSF ((INTCON) and 07Fh), 5 ;se habilita interrupción TMR0
    BCF ((INTCON) and 07Fh), 2 ;se limpia la bandera del TMR0
    RETURN
...
CONFII0CB:
    ...
    BCF ((INTCON) and 07Fh), 0 ;se limpia la bandera de PORTB
    RETURN

INTERRUPI0CB:
    BANKSEL PORTA
    BTFSS PORTB, UP ;se analiza ((PORTB) and 07Fh), 0 /(si no está presionado salta una linea)
    INCF PORTA
    BTFSS PORTB, DOWN ;se analiza ((PORTB) and 07Fh), 1/(si no está presionado salta una linea)
    DECF PORTA
    BCF ((INTCON) and 07Fh), 0 ;se limpia la bandera (de cambio de estado del PORTB)
    RETURN
